...
let addStopMode    = null;
let bboxMode       = false;
let bboxCorner1    = null;
let bboxRect       = null;   // { source updated in map } marker for cleanup
let bboxBounds     = null;   // { south, north, west, east }

let printAreaMode       = false;
let printAreaCorner1    = null;
let printAreaCornerMark = null;  // maplibregl.Marker for first corner dot
let printAreaRect       = null;
let printAreaBounds     = null;  // { south, north, west, east }

let routeCoords    = [];
let routeSegments  = [];
let routeDistM     = 0;
let routePolyline  = null;   // boolean flag: truthy when route has >=2 points
let dotMarkers     = [];
let ctxLatLng      = null;
let eraseMode      = false;
let eraseStart     = null;
let eraseMarker    = null;   // maplibregl.Marker
let stopMarkers    = [];
let _stopMarkers   = {};     // stop.id → maplibregl.Marker
...
let elevationData        = [];
let elevationDists       = [];
let routeEndpointMarkers = [];  // maplibregl.Marker instances
let _elevTimer;
let _saveTimer;
let undoStack = [];
let redoStack = [];
...
const SESSION_KEY = 'maperati_v2_session';
...
// Bounds helpers (replaces Leaflet LatLngBounds)
function makeBounds(c1, c2) {
  return {
...
function boundsContains(bounds, lat, lng) {
...
function fitMapToRoute(padding) {
  const b = getRouteBounds();
  if (!b) return;
  map.fitBounds([[b.west, b.south], [b.east, b.north]], { padding: padding || 40 });
}

// Convert [lat,lng] array to MapLibre [lng,lat]
