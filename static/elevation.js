...
async function fetchElevation() {
  ...
  try {
    const res = await fetch('/api/elevation', {
      ...
      body: JSON.stringify({coords: sampled}),
    });
...
function onElevHover(e) {
  ...
  if (elevationCoords[pos.idx]) {
    if (!_elevHoverMarker) {
      _elevHoverMarker = L.circleMarker(elevationCoords[pos.idx], {
        radius:7, color:'#fff', fillColor:'#2563EB', fillOpacity:1, weight:2, zIndexOffset:2000
      }).addTo(map);
    } else {
      _elevHoverMarker.setLatLng(elevationCoords[pos.idx]);
    }
  }
}
...
function onElevLeave() {
  ...
  if (_elevHoverMarker) { map.removeLayer(_elevHoverMarker); _elevHoverMarker = null; }
}

function onElevClick(e) {
  const pos = elevPosFromEvent(e);
  if (pos && elevationCoords[pos.idx]) map.panTo(elevationCoords[pos.idx]);
}
