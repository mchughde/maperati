...
async function loadCSV(file) {
  ...
  form.append("delimiter", document.getElementById("delimiter").value);
  try {
...
function applyFilters() {
  ...
  filteredStops = datasetStops.filter(s => {
    ...
    if (bboxBounds) {
      ...
      if (!bboxBounds.contains(ll)) return false;
    }
    return true;
  });
...
function renderDatasetMarkers() {
  ...
  filteredStops.forEach(s => {
    const m = L.circleMarker([s.lat, s.lng], {
      radius: 4, color: "#aaa", fillColor: "#ccc", fillOpacity: 0.8, weight: 1,
      pane: 'poiPane'
    }).bindTooltip(s.name);
    m.on("click", (e) => {
      L.DomEvent.stopPropagation(e);
      if (drawing) {
        addRoutePoint(s.lat, s.lng);
      } else if (!bboxMode) {
        addStop(s.name, s.lat, s.lng, s.id);
      }
    });
    datasetMarkers.addLayer(m);
  });
}
...
function clearPrintArea() {
  ...
  printAreaCorner1 = null;
  if (printAreaCornerMark) { map.removeLayer(printAreaCornerMark); printAreaCornerMark = null; }
  if (printAreaRect) { map.removeLayer(printAreaRect); printAreaRect = null; }
