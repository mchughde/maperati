...
// ── Apple Pencil tap support ──────────────────────────────
// Drawing here is tap-based: each tap adds a point (Snap = snapped to roads,
// Free = straight line to the previous point), and a drag pans the map. Safari
// doesn't reliably synthesise a click for the Apple Pencil, so we detect a
// quick pen tap and route it through onMapClick; a pen drag is left to
// MapLibre's pan. (Freehand drag-drawing was attempted but iPad Safari
// intermittently drops pointermove events mid-stroke, so tap is the reliable
// path — see git history for the overlay approach if revisiting.)
{
