...
async function addRoutePoint(lat, lng) {
  ...
  if (drawMode === 'auto') {
    ...
    try {
      const data = await apiSnapPoint(lat, lng, TRAVEL_MODES[travelMode].orsProfile, TRAVEL_MODES[travelMode].osrmProfile);
...
async function erasePoint(lat, lng) {
  ...
  if (eraseStart === null) {
    ...
    if (p1 && p2) {
      ...
      try {
        const data = await apiSnapSegment(p1, p2, TRAVEL_MODES[travelMode].orsProfile, TRAVEL_MODES[travelMode].osrmProfile);
...
async function routeViaHere(lat, lng) {
  ...
  try {
    ...
    const [d1, d2] = await Promise.all([
      apiSnapSegment(p1, [viaLat, viaLng], TRAVEL_MODES[travelMode].orsProfile, TRAVEL_MODES[travelMode].osrmProfile),
      apiSnapSegment([viaLat, viaLng], p2, TRAVEL_MODES[travelMode].orsProfile, TRAVEL_MODES[travelMode].osrmProfile),
    ]);
