...
**Why not use Maperati's built-in text directions for navigation?** Maperati's text directions are generated by routing between your stop coordinates independently — they do not follow the line you drew. Any manual routing, detour waypoints, or free-drawn sections are not reflected. Use text directions as a rough street-name guide only. For accurate, route-faithful directions, use the GPX export workflow above.
